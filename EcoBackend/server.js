...
app.get('/races', (req,res)=>{
    ...
    // const sql = `SELECT DateR, Distance, Car_id, LocationR,member_id FROM races,members,cars where Cid=Car_id and member_id=Mid and Name=${req.body.Car_name}`;
    db.query(sql, (err,data)=>{
        if(err) return res.json(err);
        return res.json(data);
    })
...
app.post('/tasks', (req,res)=>{
 ...
 app.post('/addtasks',(req,res)=>{
        ...
        const dec=req.body.DescriptionT;
        const mid=req.body.member;
        const adid=req.body.admin;
        const proid= req.body.Project_id;
        const  sdate=req.body.start_dateT;      
        const  Edate=req.body.End_date;
    
    const sql=`INSERT INTO tasks VALUES(${task},'${dec}',${mid},${adid},${proid},'${sdate}','${Edate}')`
...
app.delete('/deletetask', (req,res)=>{
    ...
    const sql = `DELETE FROM tasks WHERE Task_Number = ${req.query.task}`
    db.query(sql, (err,data)=>{
        if(err) return res.json(err);
        else{
            console.log(data);
